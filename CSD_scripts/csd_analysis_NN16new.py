...
est_xyz[1]+=cory
est_xyz[2]+=corz
indx = (est_xyz[2]<ele_pos[2,:th_start].max()+1)*(est_xyz[0]>-1.5)*(est_xyz[1]>-2)
indxt = (est_xyz[2]>2)*(est_xyz[0]<-.5)*(est_xyz[0]>-2)*(est_xyz[1]<0)

th_cut = (est_xyz[0]>ele_pos[0,-8])*(est_xyz[0]<(ele_pos[0,-8]+0.2))*(est_xyz[2]<7.2)
th_cut2 = (est_xyz[0]>ele_pos[0,-8]-2)*(est_xyz[0]<(ele_pos[0,-8]+0.4))*(est_xyz[2]<7.2)*(est_xyz[2]>2.5)*(est_xyz[1]<-1)
...
#%%
k_B=oKCSD3D(ele_pos.T, pots, own_src=est_plane, own_est=est_plane, src_type='gauss', R_init=R, lambd=1e-5)
k_C=oKCSD3D(ele_pos[:,th_start:].T, pots[th_start:], own_src=est_plane, own_est=est_plane, src_type='gauss', R_init=R, lambd=1e-5)
