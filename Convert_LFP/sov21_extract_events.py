...
trials_list =[2,4,7,11,15,17,21,28,40,44,45, 46, 51, 58, 62, 63, 69, 77, 80, 82]
trials_b = np.zeros((385,len(trials_list),300))
b,a = butter(3,[3/(Fs/2),2000/(Fs/2)], btype='bandpass')
n=0
for i in range(len(ttl)):
    t1=ttl[i]*Fs
    if i+1 in trials_list:
        trials_b[:,n] = filtfilt(b,a,sig_B1[:,int(t1-100):int(t1+500)])[:,60:360]
        n+=1
del(sig_B1)
#%%
for i in range(20):
...
lfp_s = filtfilt(b,a, lfp_s[ch_order], padlen=100)
lfp_s = np.delete(lfp_s, 287, axis=0)[:369]
ele_pos=np.delete(ele_pos, 287,axis=1)[:,:369]

all_trials = np.delete(trials_b[ch_order], 287,axis=0)[:369]
...
py.ylabel('channels')
py.subplot(212)
py.title('Correlation sov21'+lid)
...
py.savefig('sov21_'+lid+'_load')
scipy.io.savemat('./mats/sov21'+lid+'.mat',
                 dict(crtx=lfp_s[261:], th=lfp_s[:261],all_trials=all_trials,
