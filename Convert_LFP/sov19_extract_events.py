...
#%% 
loadir = 'H:\\SOVy\\SOV19\\'
name='04_ep_g0_t0.imec0.ap.bin'
lid = ''
binFullPath = Path(loadir+name)
meta = readSGLX.readMeta(binFullPath)
Fs_org = int(readSGLX.SampRate(meta))
...
for i in range(60):
    if meta['typeThis'] == 'imec':    
        trials_b[:,i] = 1e3*readSGLX.GainCorrectIM(trials_b[:,i], channels, meta)
...
lfp_s = np.delete(lfp_s, 287, axis=0)[:372]
ele_pos=np.delete(ele_pos,287,axis=1)[:,:372]
...
for part in range(frags-3):
    # cor_score[:,part] = kaji_score(lfp_s[:,leap*part:leap*part+leng], lfp_s[:,leap*part:leap*part+leng])
    pot2 = lfp_s[ch_stay,leap*part:leap*part+leng]/np.linalg.norm(lfp_s[ch_stay, leap*part:leap*part+leng])
    for ch in range(ele_pos.shape[1]):
        pot1 = lfp_s[ch,leap*part:leap*part+leng]/np.linalg.norm(lfp_s[:, leap*part:leap*part+leng])
        cor_score[ch, part] = np.inner(pot1, pot2)
...
py.ylabel('channels')
py.subplot(212)
py.title('Correlation sov19'+lid)
...
py.savefig('sov19_'+lid+'_load')
scipy.io.savemat('./mats/sov19'+lid+'.mat',
