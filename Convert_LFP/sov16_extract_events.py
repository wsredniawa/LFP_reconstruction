...
def organize_pos(ele_pos, plot=False):
    ...
    a0 = a01
    a1 = np.zeros(3)
    a8 = np.array([-1.6, 3.65, 2.25]) - a0
    h1 = np.array([-2.6, 5.4, 1.25]) - a0
    h8 = np.array([-2.6, 4.7, 2.46]) - a0
    xpos1, ypos1, zpos1 = [a1[0], a8[0], h1[0], h8[0]], [a1[1], a8[1], h1[1], h8[1]], [a1[2], a8[2], h1[2], h8[2]]
    ele_pos = organize_eles(ele_pos, 0)
    ele_pos_rot1 = rotate(ele_pos, -np.pi/4, 'z')
    ele_pos_rot2 = rotate(ele_pos_rot1, -np.pi/11, 'x')
    a12 = np.array([-1.6, 4, 1.53])-a0
    print('vector length: ', np.linalg.norm(a12))
    a82 = np.array([-1.6, 3.2, 2.8])-a0
    h12 = np.array([-2.6, 5.05, 1.78])-a0
    h82 = np.array([-2.6, 4.42, 3])-a0
    ...
    a1t = np.array([-3.6, 2.28, 6.2]) - a0
    a8t = np.array([-3.6, 2.28, 7.6]) - a0
    h1t = np.array([-3.6, 3.57, 6.2]) - a0
    h8t = np.array([-3.6, 3.57, 7.6]) - a0
    xpos2, ypos2, zpos2 = [a1t[0], a8t[0], h1t[0], h8t[0]], [a1t[1], a8t[1], h1t[1], h8t[1]], [a1t[2], a8t[2], h1t[2], h8t[2]]
    a8t2 = np.array([-3.6, 2.28, 6.6]) - a0
    a8t3 = np.array([-3.6, 2.28, 5.6]) - a0
...
def load_frag(file, start=0, stop=50, part_shape=20000, resamp=40):
    ...
    for i in range(128):
        sig_mtrx[i] = filtfilt(b,a,sig_mtrx[i]-np.mean(sig_mtrx[i]))
    return sig_mtrx
...
def draw_signal(col, depth, shift=64, draw=False):
    global events
    events = np.where(sig[-1,:]>4000)[0][::10]
    py.figure()
    py.plot(sig[-1,:])
...
def draw_signal_npy(pots, ele_pos, part,color='black'):
    ...
    for n in range(pots.shape[0]):
        py.text(ele_pos[n,0], -ele_pos[n,2], str(n))
        if part=='th':
            py.plot(time+ele_pos[n,1], pots[n]-ele_pos[n,2], color=color)
        else: 
            py.plot(time+ele_pos[n,0], pots[n]/8-ele_pos[n,2], color=color)
#%%
savedir = './npys/'
datadir = 'H:/SOVy/SOV16/'
sovs = ['01wasy_', '02wasy_', '08wasy_', '09wasy_', '10wasy_']
resamp = 4

ele_pos  = organize_pos(ele_pos_pre, plot=0)
Fs=10000
b,a= butter(3, [1/(Fs/2), 1000/(Fs/2)], btype='bandpass')
lay = pd.read_csv('A8x8.lay', sep=',', header = None)
lay_mir = pd.read_csv('A8x8_mirror.lay', sep=' ', header = None)
mapa1 = pd.read_csv('samstac40_NeuroNexus_A1_8x8_EK.map',sep=' ', header=None)
for sov in sovs:
    ...
    py.ylabel('channels')
    py.subplot(212)
    py.title('Correlation sov16:: '+savename)
    py.imshow(cor_score, vmin=-1, vmax=1, extent=[-5,25,128,1],aspect='auto', cmap='PiYG')
    py.ylabel('channels'), py.xlabel('time')
    # py.savefig('sov16_'+savename+'_load')
    py.figure()
    draw_signal_npy(trials_b.mean(axis=1)/1e2, ele_pos[:64], part,color='black')
    py.close('all')
    scipy.io.savemat('./mats/sov16_'+savename+'.mat',
